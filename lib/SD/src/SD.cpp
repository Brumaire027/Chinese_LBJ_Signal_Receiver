...
bool SDFS::spaceInfo(uint64_t &total, uint64_t &free)
{
    total = free = 0;
    ...
    if (f_getfree(drv, &clusters, &info) != FR_OK) return false;
#if _MAX_SS != 512
    const uint64_t clusterBytes = uint64_t(info->csize) * info->ssize;
#else
    const uint64_t clusterBytes = uint64_t(info->csize) * 512;
#endif
    total = clusterBytes * (info->n_fatent - 2);
    free = clusterBytes * clusters;
