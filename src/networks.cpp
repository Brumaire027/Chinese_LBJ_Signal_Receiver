...
const char *time_zone = LOCAL_TIME_ZONE;
...
void onTelnetConnect(String ip) {
    ...
    if (getValidLocalTime(&current)) {
        telnet.printf("System time is %s (UTC+8), source=%s", fmtime(current), systemTimeSource());
    } else {
        telnet.print("System time UNSET (not calibrated)");
    }
